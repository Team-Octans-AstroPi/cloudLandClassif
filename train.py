...
out_model_path = script_dir/'models'/'astropi-new-clouds.tflite'
#  specify where the labels and labelled training data are
data_dir = script_dir/'data'
labels_path = data_dir/'cloudclasses'
...
for class_id, class_name in labels.items():
    ...
    for image_path in contents(data_dir/class_name):
        image = Image.open(image_path).convert('L').resize(size, Image.NEAREST).convert('RGB')
